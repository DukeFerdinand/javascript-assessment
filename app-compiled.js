...
var OpenWeather = {
    ...
    state: {
        ...
        */
        ...
        cityArray: ['New York', 'Austin', 'Houston'],
        unitObjects: [{ name: 'imperial', unitSymbol: 'F', opposite: 'metric' }, { name: 'metric', unitSymbol: 'C', opposite: 'imperial' }],
        unitSystem: 'imperial',
        unitSymbol: 'F',
    ...
    clearCurrentWeather: function clearCurrentWeather() {
    ...
    clearLoadingSpinner: function clearLoadingSpinner() {
        $('#main.row').empty();
    },
    ...
    getCityWeather: function getCityWeather() {
        ...
        var city = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.state.currentCity;

        this.clearCurrentWeather();

        $.get('http://api.openweathermap.org/data/2.5/forecast/daily?appid=' + this.state.appid + ('&q=' + city + '&mode=json&units=' + this.state.unitSystem + '&cnt=5')).done(function (results) {
            return _this.setCityWeather(results);
        }).fail(function (results) {
            return _this.renderError(results.responseJSON);
        });
    },


    //To aid in separating concerns and Quality of Life, separated all these functions
    setCityWeather: function setCityWeather(results, error) {
    ...
    grammarize: function grammarize(description) {
        var grammarizedDesc = description.split('');
    ...
    mapDailyWeatherToHTML: function mapDailyWeatherToHTML(dailyWeatherArr) {
        var _this2 = this;

        //Added grammarize method to String object
        String.grammarize = this.grammarize;

        dailyWeatherArr.forEach(function (day, index) {
            var date = new Date();
            var dayIndex = date.getDay() + index;

            $('#main.row').append('<div id=\'day' + index + '\' class=\'day col-sm-4\'>');
            var averageTemp = _this2.averageDailyTemp(day.temp);

            var description = String.grammarize(day.weather[0].description);

            //Basic templating using ES6 interpolation
            //This renders the content for each day of the week
            $('#day' + index).append('<h4 id=\'name\' class=\'text-center\'>' + _this2.state.days[dayIndex] + '</h4>' + ('<h2 class=\'text-center\'>' + day.weather[0].main + '</h2>') + ('<p class=\'text-center\'>' + description + '</p>') + ('<img class=\'img-responsive center-block\' src=http://openweathermap.org/img/w/' + day.weather[0].icon + '.png />') + ('<h4 class=\'text-center\'>' + averageTemp + ' &#176;' + _this2.state.unitSymbol + '</h4>'));
        });
    },
    ...
    */
    averageDailyTemp: function averageDailyTemp(dailyTemp) {
        var dt = dailyTemp;
        return ((dt.day + dt.eve + dt.morn + dt.night) / 4).toFixed(0);
    },
    renderWeatherData: function renderWeatherData(weatherData) {
    ...
    renderError: function renderError(error) {
        this.clearLoadingSpinner();
        $('#main.row').append('<h1>Sorry, that\'s an error :(</h1>' + ('<p>Details: ' + error.message + '</p>'));
    },


    //Handles Changing the system used from Imperial to Metric and vice-versa
    changeTempSystem: function changeTempSystem() {
        var _this3 = this;

        this.state.unitObjects.map(function (system) {
            $('#' + system.name).click(function () {
                _this3.state.unitSystem = system.name;
                _this3.state.unitSymbol = system.unitSymbol;
                _this3.getCityWeather();
            });
        });
    },
    renderDropdown: function renderDropdown() {
        ...
        $('div.container').prepend('<div class=\'row\'>' +
        //Unit changing buttons
        '<button id=\'imperial\' class=\' btn btn-default col-xs-2\'>Imperial</button>' + '<button id=\'metric\' class=\'btn btn-default col-xs-2 \'>Metric</button>' +

        //Dropdown menu
        '<div class="col-xs-2 dropdown">' + '<button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown">More Cities' + '<span class="caret"></span></button>' + '<ul class="dropdown-menu">' + '<li id=\'dd0\'>New York, NY</li>' + '<li id=\'dd1\'>Austin, TX</li>' + '<li id=\'dd2\'>Houston, TX</li>' + '</ul>' + '</div>' + '</div>');
